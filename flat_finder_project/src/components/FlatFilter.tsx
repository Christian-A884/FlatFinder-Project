...
const FlatFilter = () => {
  ...
  const filterFlat = (flat: Flat[]) => {
    return flat.filter((item) => {
      return (
        (flatFilter.city === "" ||
          item.city.toLowerCase().includes(flatFilter.city.toLowerCase())) &&
        (flatFilter.minPrice === "" ||
          item.rentPrice >= parseFloat(flatFilter.minPrice)) &&
        (flatFilter.maxPrice === "" ||
          item.rentPrice <= parseFloat(flatFilter.maxPrice)) &&
        (flatFilter.minArea === "" ||
          item.areaSize >= parseFloat(flatFilter.minArea)) &&
        (flatFilter.maxArea === "" ||
          item.areaSize <= parseFloat(flatFilter.maxArea))
      );
    });
  };
  ...
  const sortFlat = (flat: Flat[]) => {
    return flat.slice().sort((a, b) => {
      ...
      if (typeof fieldA === "string" && typeof fieldB === "string") {
        return flatSort.order === "Ascending"
      ...
      <div className="flex justify-center md:justify-start md:flex-col xl:flex-row gap-5">
        ...
        <div className="flex flex-col justify-start items-start w-full md:flex-row px-4 gap-3">
          <div className="flex flex-col text-sm gap-1">
            ...
            <select
              className="border text-gray-400 text-sm"
              name="criteria"
              value={flatSort.criteria}
              onChange={(e) => handleFlatSortChange(e)}
            >
