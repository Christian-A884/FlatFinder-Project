...
const UserFilter = ({ users, setUsers }:UserFilterProps) => {
    ...
    return user.filter((item) => {
      return (
        (userFilter.userType === "" ||
          item.role
            .toLowerCase()
            .includes(userFilter.userType.toLowerCase())) &&
        (userFilter.minAge === "" ||
          item.age >= parseFloat(userFilter.minAge)) &&
        (userFilter.maxAge === "" ||
          item.age <= parseFloat(userFilter.maxAge)) &&
        (userFilter.minFlat === "" ||
          item.userFlatCount >= parseFloat(userFilter.minFlat)) &&
        (userFilter.maxFlat === "" ||
          item.userFlatCount <= parseFloat(userFilter.maxAge))
      );
    });
  };
  ...
  const sortUser = (user: User[]) => {
    return user.slice().sort((a, b) => {
      ...
      if (typeof fieldA === "string" && typeof fieldB === "string") {
        return userSort.order === "Ascending"
  ...
  const handleUserSort = () => {
    ...
    console.log("sorted", sortUser(users));
  };
        ...
        <div className="flex flex-col justify-start items-start w-full md:flex-row px-4 gap-3">
          <div className="flex flex-col text-sm gap-1">
            ...
            <select
              ...
              name="criteria"
              value={userSort.criteria}
              onChange={(e) => handleUserSortChange(e)}
            >
