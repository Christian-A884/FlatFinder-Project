          ...
          {flat.map((obj, index, btnIndex) => (
            <tr key={index}>
              {column.map((columnItem, colIndex) => (
                <td
                  key={colIndex}
                  className="text-sm font-semibold text-[#173466] text-center justify-center px-2 py-1"
                >
              ...
              <td key={btnIndex}>
                <div className="flex gap-3 items-center justify-center p-2 ">
                  <NavLink
                      to={`/flat-view/${obj.id}`}  className="text-[8px] w-16 text-center bg-[#F1654D] p-1 rounded-md text-xs text-white font-semibold">
                    View flat
                  </NavLink>

                  <button onClick={()=> handleClick(obj.id)} className={`text-xs  ${ favFlat.some(item => item.flatId === obj.id) ? "bg-blue-600" : "bg-[#F1654D]"}  p-1 rounded-md text-white font-semibold`}>
                    Favourite
                   </button>
